...
{
#pragma warning disable IDE0079 // 不需要移除未使用的代码，因为这些代码是为了与 Windows API 交互而定义的接口和常量。
#pragma warning disable IDE0028 // 不需要将成员转换为属性，因为这些成员是为了与 Windows API 交互而定义的接口和常量。
#pragma warning disable IDE0054 // 不需要将成员转换为复合赋值，因为这些成员是为了与 Windows API 交互而定义的接口和常量。
#pragma warning disable IDE0090 // 不需要将成员转换为本地函数，因为这些成员是为了与 Windows API 交互而定义的接口和常量。

#pragma warning disable SYSLIB1054 // 不需要将成员标记为可本地化，因为这些成员是为了与 Windows API 交互而定义的接口和常量。
#pragma warning disable SYSLIB1096 // 不需要将成员标记为可本地化，因为这些成员是为了与 Windows API 交互而定义的接口和常量。

#pragma warning disable CA1050 // 在命名空间中声明类型
#pragma warning disable CA1826 // 不需要将成员标记为静态，因为这些成员是为了与 Windows API 交互而定义的接口和常量。
#pragma warning disable CS8618 // 非 null 的字段必须包含非 null 的值。请考虑将字段声明为可以为 null。
